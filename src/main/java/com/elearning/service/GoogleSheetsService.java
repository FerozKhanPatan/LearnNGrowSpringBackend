...
public class GoogleSheetsService {
    ...
    public void addEnrollmentToSheet(Enrollment enrollment, String name, String email) {
        ...
        try {
            // Load credentials from environment variable (LAZY)
            System.out.println("STEP 1: Loading credentials from environment variable...");
            String credentialsJson = System.getenv("GOOGLE_CREDENTIALS_JSON");
            
            if (credentialsJson == null || credentialsJson.isEmpty()) {
                System.err.println("ERROR: GOOGLE_CREDENTIALS_JSON env variable not found or empty");
                throw new RuntimeException("GOOGLE_CREDENTIALS_JSON env variable not found");
            }
            
            System.out.println("GOOGLE_CREDENTIALS_JSON env variable found (length: " + credentialsJson.length() + " characters)");
            
            InputStream credentialsStream = new ByteArrayInputStream(
                credentialsJson.getBytes(StandardCharsets.UTF_8)
            );
            
            System.out.println("Credentials stream created from environment variable");
            ...
            System.out.println("Service account ID: " + credential.getServiceAccountId());
                System.out.println("STEP 1 COMPLETED: Credentials loaded successfully from environment variable");
